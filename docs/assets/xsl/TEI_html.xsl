...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="generatePageStructure">
        ...
        <html lang="sv" xml:lang="sv">
            <head>
                ...
                <link rel="stylesheet" href="{concat($css, 'desktop.css')}"/> 
                <style>
                    body { padding-top: 20px; padding-bottom: 20px; font-family: sans-serif; }
                    .content-section { padding: 20px; margin-bottom:20px; background-color:#f9f9f9; border-radius:5px; }
                    .image-gallery-item { margin-bottom: 30px; }
                    .img-preview { max-width: 100%; height: 200px; object-fit: cover; border-radius: 4px; border: 1px solid #ddd; margin-bottom:10px; }
                    nav.main-nav a.nav-link { margin-right: 15px; color: #007bff; }
                    nav.main-nav a.active { font-weight: bold; text-decoration: underline; color: #0056b3;}
                    header.page-header h1 { font-size: 2.5rem; margin-bottom: 0.5rem; }
                    header.page-header .lead { font-size: 1.25rem; color: #6c757d; }
                    footer { font-size: 0.9em; color: #6c757d; }
                    .metadata-block { margin-top: 15px; }
                    .metadata-block h4 {font-size: 1.2em; color: #333; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-bottom: 10px;}
                    .metadata-block p {margin-bottom: 0.3rem;}
                ...
                <xsl:for-each select="$corpusNode/tei:TEI">
                    ...
                    <xsl:variable name="image_display_url">
                        <xsl:choose>
                            ...
                            <xsl:when test="$publish_graphic_node/@url">
                                <xsl:value-of select="concat($image, $publish_graphic_node/@url)"/>
                            </xsl:when>
                            <xsl:otherwise>#bild_saknas.png</xsl:otherwise>
                        </xsl:choose>
                    </xsl:variable>
            ...
            <div class="col-md-4">
                <article class="metadata-section">
                    ...
                    <xsl:if test="$metadata_file_ref_path_in_tei">
                        <xsl:variable name="metadata_doc" select="document($metadata_file_ref_path_in_tei, $corpusNode)"/>
                        <xsl:if test="$metadata_doc">
                            <div class="metadata-block">
                                <h4>Teknisk metadata (från <xsl:apply-templates select="$imageNode/tei:teiHeader/tei:fileDesc/tei:sourceDesc/tei:p/tei:ref[@type='application/rdf+xml']"/>)</h4>
                                <p><small>
                        ...
                        <xsl:if test="not($metadata_doc)">
                            <p><small>Kunde inte ladda teknisk metadata från "<xsl:value-of select="$metadata_file_ref_path_in_tei"/>". Kontrollera att sökvägen är korrekt relativt till TEI-filen och att filen existerar.</small></p>
                        </xsl:if>
                    </xsl:if>
                     <div class="metadata-block">
    ...
    <xsl:template match="tei:persName | tei:orgName | tei:name">
        <xsl:choose>
            ...
            <xsl:when test="@ref and starts-with(@ref, '#')"> <xsl:apply-templates/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:apply-templates/>
            </xsl:otherwise>
